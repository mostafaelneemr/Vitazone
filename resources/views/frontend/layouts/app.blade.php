...
@if(\App\Language::where('code', Session::get('locale', Config::get('app.locale')))->first()->rtl == 1)
    ...
    @else
        ...
        <head>
            ...
            @if (\App\BusinessSetting::where('type', 'google_tag')->first()->value == 1)
                {!! \App\BusinessSetting::where('type', 'google_tag_script')->first()->value !!}
            @endif
            ...
            @php
                ...
                $keyword_seo = $seosetting->{'keyword_'.locale()};
                $title_seo = $seosetting->{'title_'.locale()};
            @endphp
            ...
            <title>@yield('meta_title', $title_seo)</title>
            <meta name="description" content="@yield('meta_description', $seosetting->{'description_'.locale()})" />
            <meta name="keywords" content="@yield('meta_keywords', $keyword_seo)">
            <meta name="author" content="{{ $seosetting->{'author_'.locale()} }}">
            <meta name="sitemap_link" content="{{ $seosetting->sitemap_link }}">

            @yield('meta')

                <meta itemprop="image" content="{{ uploaded_asset(\App\GeneralSetting::first()->$logo) }}">

                <!-- Twitter Card data -->
                <meta name="twitter:card" content="product">
                <meta name="twitter:site" content="@publisher_handle">
                <meta name="twitter:creator" content="@author_handle">
                <meta name="twitter:image" content="{{ uploaded_asset(\App\GeneralSetting::first()->$logo) }}">

                <!-- Open Graph data -->
                <meta property="og:type" content="website" />
                <meta property="og:url" content="{{ route('home') }}" />
                <meta property="og:image" content="{{ uploaded_asset(\App\GeneralSetting::first()->$logo) }}" />
        ...
        <body>
            ...
            @if (\App\BusinessSetting::where('type', 'google_tag')->first()->value == 1)
                {!! \App\BusinessSetting::where('type', 'google_tag_tag')->first()->value !!}
            @endif
        ...
        @if (\App\BusinessSetting::where('type', 'tawk_chat')->first()->value == 1)
        <script type="text/javascript">
